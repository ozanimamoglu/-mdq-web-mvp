...
if (watch.productImage !== null) {
  ...
  if (!hasUsableWatchMarketPrice(watch)) {
    return false;
  }
  ...
  if (
    !Number.isInteger(watch.evidenceCount) ||
    watch.evidenceCount < 1
  ) {
    return false;
  }

  if (!isNonEmptyString(watch.evidenceUnit)) {
    return false;
  }

  if (!isNonEmptyString(watch.evidenceLastUpdated)) {
    return false;
  }
  ...
  if (!isNonEmptyString(watch.evidenceMethod)) {
    return false;
  }
  ...
  const integrity = watch.productIntegrity;
  ...
  for (const question of watch.questions) {
    if (
      !question ||
